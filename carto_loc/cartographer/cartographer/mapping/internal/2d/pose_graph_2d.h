...
//它被扩展到子地图构建:
//每个节点都被一个或多个子地图匹配(为每个匹配添加一个约束)，节点的位置和子地图的位置都要优化。
//所有约束都在子地图i和节点j之间。
...
//   this submap for the last time.
 ...
 private:
  ...
  std::map<std::string, PoseGraphInterface::LandmarkNode> all_landmark_nodes_;

  ValueConversionTables conversion_tables_;

  //penghu*
  std::thread* updatemapdata_thread_;
  void UpdateMapData() LOCKS_EXCLUDED(mutex_);
