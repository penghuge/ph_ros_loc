...
//这让人想起std::map，但是用'IdType'索引，它可以是'NodeId'或'SubmapId'。
//注意:这个容器将永远只包含非空轨迹。修剪一条trajectory的最后一个剩余节点会导致丢弃trajectory
...
class MapById {
 ...
 public:
  ...
  // before 'time'.
  //返回一个指向'trajectory_id'对应的轨迹下容器的第一个元素的迭代器，该元素的时间不在'time'之前，
  //或者如果所有的键都在'time'之前，则返回 EndOfTrajectory(trajectory_id)。
  ConstIterator lower_bound(const int trajectory_id,
                            const common::Time time) const {
    //不存在 trajectory_id 对应的轨迹，直接返回
    ...
    //如果轨迹中最近一帧数据的时间戳都比time早，即所有的键都在'time'之前，直接返回
