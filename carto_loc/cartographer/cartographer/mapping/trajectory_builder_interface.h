...
//该接口同时用于2D和3D SLAM。实现连接一个全局SLAM堆栈，即用于初始姿态估计的局部SLAM，
//用于检测环路闭合的扫描匹配，以及用于计算优化姿态估计的稀疏位姿图优化。
class TrajectoryBuilderInterface {
 public:

  //用来保存插入Local Slam的一个节点的数据结构
  //存储一帧激光数据插入到submap中的结果
  ...
  //lyy
  virtual void AddSubmapIdAndFlag(int submap_id, bool assigned_flag) = 0;
