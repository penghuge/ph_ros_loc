...
void SensorBridge::HandleLaserScanMessage(
  ...
  // 诊断：确认 LaserScan 原始 beam 数与转换后点数的差异（1Hz）。
  static carto::common::Time last_log_time = carto::common::Time::min();
  if (last_log_time == carto::common::Time::min() ||
      time - last_log_time >= carto::common::FromSeconds(1.0)) {
    last_log_time = time;
    LOG(INFO) << "[SensorBridge] LaserScan->PointCloud";
    LOG(INFO) << "  sensor_id=" << sensor_id << " frame_id=" << msg->header.frame_id
              << " stamp=" << msg->header.stamp;
    LOG(INFO) << "  laser_ranges_total=" << msg->ranges.size()
              << " angle_min=" << msg->angle_min << " angle_max=" << msg->angle_max
              << " range_min=" << msg->range_min << " range_max=" << msg->range_max;
    LOG(INFO) << "  point_cloud.points=" << point_cloud.points.size()
              << " point_cloud.intensities=" << point_cloud.intensities.size();
  }
...
void SensorBridge::HandleLaserScan(
  ...
  // 诊断：确认 LaserScan subdivision 后实际送入 Rangefinder 的点数（1Hz）。
  static carto::common::Time last_subdivision_log_time =
    carto::common::Time::min();
  if (last_subdivision_log_time == carto::common::Time::min() ||
    subdivision_time - last_subdivision_log_time >=
      carto::common::FromSeconds(1.0)) {
    last_subdivision_log_time = subdivision_time;
    LOG(INFO) << "[SensorBridge] LaserScan subdivision";
    LOG(INFO) << "  sensor_id=" << sensor_id << " frame_id=" << frame_id
        << " points_total=" << points.points.size()
        << " subdivision=" << (i + 1) << "/" << num_subdivisions_per_laser_scan_
        << " subdivision_points=" << subdivision.size();
  }
...
void SensorBridge::HandleRangefinder(
  ...
  // 诊断：确认最终送入 Cartographer TrajectoryBuilder 的点数（1Hz）。
  static carto::common::Time last_rangefinder_log_time =
    carto::common::Time::min();
  if (last_rangefinder_log_time == carto::common::Time::min() ||
    time - last_rangefinder_log_time >= carto::common::FromSeconds(1.0)) {
    last_rangefinder_log_time = time;
    LOG(INFO) << "[SensorBridge] Rangefinder input";
    LOG(INFO) << "  sensor_id=" << sensor_id << " frame_id=" << frame_id
        << " timed_points=" << ranges.size();
  }
    ...
    trajectory_builder_->AddSubmapIdAndFlag(submap_id_, assigned_flag_);//lyy
    trajectory_builder_->AddSensorData(
