#ifndef RIGID_TRANSFORMER_H_
#define RIGID_TRANSFORMER_H_
...
class Rigid2 {
 public:
  ...
  double normalized_angle() const {
    return NormalizeAngle(rotation().angle());
  }
  ...
  std::string NumToString(const double num, int decplaces = 3)
  {
  ...
  std::string DebugString() const {
    return std::string("{ t: [") + std::to_string(translation().x())
            + std::string(", ") + std::to_string(translation().y())
            + std::string("], r: [")  + std::to_string(rotation().angle())
            + std::string("] }");
  }
...
std::ostream& operator<<(std::ostream& os,
                         const SlamCommon::Rigid2<T>& rigid) {
...
class Rigid3 {
 public:
  ...
  std::string DebugString() const {
    return std::string("{ t: [") + std::to_string(translation().x())
            + std::string(", ") + std::to_string(translation().y())
            + std::string(", ") + std::to_string(translation().z())
            + std::string("], q: [")  + std::to_string(rotation().w())
            + std::string(", ") + std::to_string(rotation().x())
            + std::string(", ") + std::to_string(rotation().y())
            + std::string(", ") + std::to_string(rotation().z())
            + std::string("] }");
  }
...
template <typename T>
std::ostream& operator<<(std::ostream& os,
                         const SlamCommon::Rigid3<T>& rigid) {
  os << rigid.DebugString();
  return os;
}
