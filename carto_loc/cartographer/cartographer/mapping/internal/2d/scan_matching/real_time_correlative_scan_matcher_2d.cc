...
float ComputeCandidateScore(const ProbabilityGrid& probability_grid,
                            ...
                            int x_index_offset, int y_index_offset) {
  float candidate_score = 0.f;
  // float candidate_weight = 0.f;//lyy
  for (const Eigen::Array2i& xy_index : discrete_scan) {
    const Eigen::Array2i proposed_xy_index(xy_index.x() + x_index_offset,
                                           xy_index.y() + y_index_offset);
    const float probability =
        probability_grid.GetProbability(proposed_xy_index);
    // candidate_weight += probability;//lyy
    // candidate_score += probability*probability;
    candidate_score += probability;
  }
  candidate_score /= static_cast<float>(discrete_scan.size());
  // if(candidate_weight > 0.0f){
  //   candidate_score /= candidate_weight;
  // }else{
  //   candidate_score = candidate_weight / static_cast<float>(discrete_scan.size());
  // }
  // LOG_INFO("real csm score: %f", candidate_score);//lyy
...
double RealTimeCorrelativeScanMatcher2D::Match(
  ...
  // LOG_INFO("best_candidate.x,y1(%f, %f, %f)", best_candidate.x, best_candidate.y, best_candidate.score);
