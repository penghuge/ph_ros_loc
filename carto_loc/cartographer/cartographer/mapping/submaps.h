...
// closing.
//一个单独的submap(在局部map帧中有一个'local_pose')记录有多少测量数据被插入其中，
//并在map不再改变并准备好结束循环时设置'insertion_finished'。
class Submap {
 public:
 ...
 private:
  const transform::Rigid3d local_pose_;//目前理解的local_pose其实是激光在tracking坐标系下的坐标？
