...
LaserScanToPointCloudWithIntensities(const LaserMessageType& msg) {
  ...
  // 诊断：统计 LaserScan beams 在转换为点云时的过滤情况（1Hz）。
  // 这里是 2400→8xx 的关键位置：只有通过 HasEcho 且在 [range_min, range_max]
  // 内的 beam 才会被转换成点。
  static ::cartographer::common::Time last_log_time =
      ::cartographer::common::Time::min();
  const ::cartographer::common::Time msg_time = FromRos(msg.header.stamp);
  const bool do_log = (last_log_time == ::cartographer::common::Time::min()) ||
                      (msg_time - last_log_time >=
                       ::cartographer::common::FromSeconds(1.0));
  if (do_log) {
    last_log_time = msg_time;
  }

  size_t below_min_cnt = 0;
  size_t above_max_cnt = 0;
  size_t inf_cnt = 0;
  size_t nan_cnt = 0;
  size_t huge_cnt = 0;
  ...
  for (size_t i = 0; i < msg.ranges.size(); ++i) {
    ...
    if (HasEcho(echoes)) {
      ...
      // 诊断：无回波/异常值统计。
      // LaserScan 常把无回波表示为 +inf（或极大值），这会导致 above_max 很大。
      if (std::isnan(first_echo)) {
        ++nan_cnt;
      } else if (std::isinf(first_echo)) {
        ++inf_cnt;
      } else if (first_echo > 1e3f) {
        // 经验阈值：大到不可能是有效激光量测（基本可以当作无回波/异常）。
        ++huge_cnt;
      }
      ...
      if (msg.range_min <= first_echo && first_echo <= msg.range_max) {
        ...
        if (first_echo < msg.range_min) {
          ++below_min_cnt;
        } else if (first_echo > msg.range_max) {
          ++above_max_cnt;
        }
      }
    }
  ...
  if (do_log) {
    LOG(INFO) << "[msg_conversion] LaserScanToPointCloudWithIntensities"
            << " frame_id=" << msg.header.frame_id
            << " stamp=" << msg.header.stamp
            << " total=" << msg.ranges.size()
            << " accepted=" << point_cloud.points.size()
            << " below_min=" << below_min_cnt
            << " above_max=" << above_max_cnt
            << " inf=" << inf_cnt
            << " nan=" << nan_cnt
            << " huge=" << huge_cnt
...
template <typename LaserMessageType>
::cartographer::sensor::PointCloud LaserScanToPointCloud(const LaserMessageType& msg) {
  CHECK_GE(msg.range_min, 0.f);
  CHECK_GE(msg.range_max, msg.range_min);
  if (msg.angle_increment > 0.f) {
    CHECK_GT(msg.angle_max, msg.angle_min);
...
::cartographer::sensor::PointCloud ToPointCloud(const sensor_msgs::LaserScan& msg) {
  return LaserScanToPointCloud(msg);
}
