...
class MapBuilderBridge {
 public:
  ...
  MapBuilderBridge(
      ...
      tf2_ros::Buffer* tf_buffer, SlamCommon::CFileInterface* file_interface);
  ...
  float GetBestScore() LOCKS_EXCLUDED(mutex_);
  int GetMatchedSubmapId() LOCKS_EXCLUDED(mutex_);
  visualization_msgs::MarkerArray GetTrajectoryNodeList();
  visualization_msgs::MarkerArray GetLandmarkPosesList();
  visualization_msgs::MarkerArray GetConstraintList();

  SensorBridge* sensor_bridge(int trajectory_id);
  bool SetInitPoseToLocal(SlamCommon::Pose3D init_pose);
