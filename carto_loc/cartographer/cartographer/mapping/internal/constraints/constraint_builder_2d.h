...
//混合任意数量的调用'MaybeAddConstraint'， 'MaybeAddGlobalConstraint'，和'NotifyEndOfNode'，
//然后调用'WhenDone'一次。在所有的计算完成后，调用'callback'处理执行结果，
//下一次MaybeAdd(Global)Constraint()/WhenDone()循环则可以继续。
//这个类是线程安全的
