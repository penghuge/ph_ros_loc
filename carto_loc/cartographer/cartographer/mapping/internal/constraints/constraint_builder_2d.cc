...
void ConstraintBuilder2D::ComputeConstraint(
  ...
  //构建子地图和轨迹节点之间的约束之前，先利用分支界定法进行快速匹配，消除不正确的匹配
  ...
  if (match_full_submap) {
    ...
    //闭环检测数据不够好时，不再用ceres solver进行位姿优化
...
void ConstraintBuilder2D::RunWhenDoneCallback() {
  ...
  std::unique_ptr<std::function<void(const Result&)>> callback;
  {
    ...
    constraints_.clear();
    //将when_done_的权限转给callback，重置when_done_
    callback = std::move(when_done_);
    when_done_.reset();
    //记录约束长度，放在constraints_.clear()之后记录，不应该一直是0？是否要调换位置
    kQueueLengthMetric->Set(constraints_.size());
  }
  //将剩余的约束调用when_done_传入的方法，执行优化
  (*callback)(result);
}
