...
MapBuilderBridge::MapBuilderBridge(
    ...
    tf2_ros::Buffer* const tf_buffer, SlamCommon::CFileInterface* file_interface)
    : node_options_(node_options),
      map_builder_(std::move(map_builder)),
      tf_buffer_(tf_buffer),
      best_score_(0.0f), matched_submap_id_(-1), file_interface_(file_interface)
      {}

bool MapBuilderBridge::SetInitPoseToLocal(SlamCommon::Pose3D init_pose)
{
    if(map_builder_->Setinitpose(init_pose)){
        return true;
    }
...
void MapBuilderBridge::HandleRefInSubmapQuery(
  common_msgs::SetPose::Request& request, 
  common_msgs::SetPose::Response& response) {
  response.result.clear();            

  file_interface_->SetRefColsPosition(request.x, request.y);
  response.result = file_interface_->GetRefColsResponseResults();

  if(response.result.empty()){
    response.result = "none";
  }else {
    int ref_id = round(request.theta);
    response.result = absl::StrCat(ref_id, " ", response.result);

    LOG(INFO) << "HandleRefInSubmapQuery: request (" << request.x << ", " << request.y << ", " << request.theta << "), result: " << response.result;
  }
}
...
float MapBuilderBridge::GetBestScore(){
  return best_score_;
}

int MapBuilderBridge::GetMatchedSubmapId() {
  return matched_submap_id_;
}
