...
std::set<SubmapId> AddSubmapsToSubmapCoverageGrid2D(
    ...
    for (const auto& submap : submap_data) {
        ...
        for (const Eigen::Array2i& xy_index : XYIndexRangeIterator(cell_limits)) {
            ...
            //LOG_INFO("cell index valid 1");
            if (!grid.IsKnown(index)) continue;
            //LOG_INFO("cell index valid 1 really");
...
std::map<SubmapId, common::Time> ComputeSubmapFreshness(
    ...
    for (const PoseGraphInterface::Constraint& constraint : constraints) {
        //LOG_INFO("constraints size = %d", constraints.size());
...
void OverlappingSubmapsTrimmer2D::Trim(Trimmable* pose_graph) {
    ...
    //LOG_INFO("submap_data.size() - current_submap_count_ = (%d - %d)", submap_data.size(), current_submap_count_);
    ...
    //LOG_INFO("submap_ids_to_remove_SIZE = %d", submap_ids_to_remove.size());
    for (const SubmapId& id : submap_ids_to_remove) {
        //LOG_INFO("is triming traj_id = %d, submap_index = %d", id.trajectory_id, id.submap_index);
