 ...
 */
 ...
 DEFINE_string(map_name, "map", "Stem of the output files' name.");
 ...
 //   WritePgm(image, resolution, &pgm_writer);
   WritePgm(FLAGS_map_name,image, resolution, &pgm_writer);
   ...
   // WriteXml(map_filestem, map_filestem +".xml", resolution, origin,m_config_oper);
   WriteXml(map_filestem, map_filestem +".xml", resolution, origin);
 ...
 int main(int argc, char** argv) {
   ...
   CHECK(!FLAGS_map_name.empty())  <<"-map_name is missing.";
