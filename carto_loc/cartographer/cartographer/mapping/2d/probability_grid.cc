...
//将调用ComputeLookupTableToApplyOdds()时指定的'odds'应用到'cell_index'处的单元格的概率(如果该单元格还没有被更新)。
//同一个单元格的多次更新将被忽略，直到调用FinishUpdate()。如果单元格被更新，返回true。
//如果这是对指定单元格的第一次调用ApplyOdds()，则它的值将被设置为与“odds”对应的概率。
bool ProbabilityGrid::ApplyLookupTable(const Eigen::Array2i& cell_index,
  ...
  *cell = table[*cell];//查询表中的值被累加过kUpdateMarker，用于标记该栅格待处理
