...
bool FastCorrelativeScanMatcher2D::MatchWithSearchParameters(
  ...
  if (best_candidate.score > min_score) {
    *score = best_candidate.score;
    // LOG_INFO("best_candidate.x,y2(%f, %f)", best_candidate.x, best_candidate.y);
    LOG_INFO("fast csm score: %f", best_candidate.score);//lyy
...
void FastCorrelativeScanMatcher2D::ScoreCandidates(
  ...
  for (Candidate2D& candidate : *candidates) {
    ...
    for (const Eigen::Array2i& xy_index :
      ...
      // int tmp_value = precomputation_grid.GetValue(proposed_xy_index);//lyy
      sum += precomputation_grid.GetValue(proposed_xy_index);
      // weight += tmp_value;
      // sum += tmp_value * tmp_value;
    }
    candidate.score = precomputation_grid.ToScore(
        sum / static_cast<float>(discrete_scans[candidate.scan_index].size()));
    // if(weight > 0){//lyy
    //   candidate.score = precomputation_grid.ToScore(sum / weight);
    // }else{
    //   candidate.score = precomputation_grid.ToScore(weight / static_cast<float>(discrete_scans[candidate.scan_index].size()));
    // }
