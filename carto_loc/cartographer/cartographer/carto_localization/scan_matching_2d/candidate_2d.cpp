...
{
    // set this value to something on the order of resolution to make sure that
    // the std::acos() below is defined.
    //设定最小的最大范围，是为了下面计算acos有意义
    ...
    // angular_perturbation_step_size *= 8.0;//lyy: angular resolution
                                   ...
                                   const CellLimits& cell_limits)
{
    ...
    for (int i = 0; i != num_scans; ++i)
    {
        ...
        {
            // The rightmost and bottommost to the (0,0).
            min_bound = min_bound.min(-xy_index);
            // The leftmost and topmost to the (num_x_cells - 1, num_y_cells - 1).
