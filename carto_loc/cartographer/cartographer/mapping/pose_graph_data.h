...
// that state.
//后台线程中子地图的当前状态。在转换到“kFinished”之后，所有节点都将尝试与这个子地图匹配。
//同样地，所有的新节点都会与该状态下的所有子地图进行匹配。
...
struct InternalSubmapData {
  ...
  // becomes 'kFinished'.
  //插入到这张地图中的节点的id以及它们的约束。当这个submap变成“kFinished”时，它们将不再被匹配。
...
struct PoseGraphData {
  ...
  //一旦看到子地图，就会立即为它们分配一个ID和状态，甚至在它们参与后台计算之前也是如此。
  ...
  // Set of all initial trajectory poses.保存了初始位姿
