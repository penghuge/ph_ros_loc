...
//根据激光点和超量程点，更新hit和miss两张栅格地图的概率
void CastRays(const sensor::RangeData& range_data,
  ...
  const double superscaled_resolution = limits.resolution() / kSubpixelScale;//高精度分辨率
  //生成一份精度提高kSubpixelScale倍的栅格
...
void ProbabilityGridRangeDataInserter2D::Insert(
  ...
  //如果在激光点插入后没有完成更新，我们将赋予激光点优先级(例如，没有激光点会因为同一个单元格中的一次丢失而被过滤)。
  CastRays(range_data, hit_table_, miss_table_, options_.insert_free_space(),
           probability_grid);
  //将栅格地图中的概率恢复到[1, 32768]范围
