...
// known cells.
//填写“offset”和“limits”以定义包含所有已知单元格的子区域。
...
//必要时增长地图以包含“点”。这改变了这些坐标的意义。
//这个方法必须在'FinishUpdate'之后立即调用，在任何调用'ApplyLookupTable'之前调用。
...
void Grid2D::GrowLimits(const Eigen::Vector2f& point,
  ...
  while (!limits_.Contains(limits_.GetCellIndex(point))) {
    ...
    const int offset = x_offset + stride * y_offset;//旧submap左上角相对于新submap左上角的偏置
