...
MapBuilder::MapBuilder(SlamCommon::CFileInterface *file_interface,
                       SlamCommon::CConfigFileOperator *config_operator,
                       const proto::MapBuilderOptions& options)
    : m_file_interface(file_interface), m_config_operator(config_operator), options_(options), thread_pool_(options.num_background_threads()) {
    ...
    local_to_global_vec_.clear();
    submap2d_protos_.clear();
}
//penghu 24/4/12
std::unique_ptr<LocalTrajectoryBuilder2D> MapBuilder::local_trajectory_ = nullptr;
::cartographer::mapping::LocalTrajectoryBuilder2D* MapBuilder::Gettraj()
{
    return local_trajectory_.get();
}
int MapBuilder::AddTrajectoryBuilder(
    ...
    if (options_.use_trajectory_builder_3d()) {
        ...
        //构造局部路径生成器，包括构造活动地图对、运动滤波器、实时相对匹配器、测距数据采集器、
        //测距数据插入器、ceres匹配器
        ...
        if (trajectory_options.has_trajectory_builder_2d_options()) {
            ...
            if(m_match_model_ == 1 || m_match_model_ == 3)
            {
                ...
                local_trajectory_builder->SetInitialTrajectoryPose(
                            trajectory_id, initial_trajectory_pose.to_trajectory_id(),
                            transform::ToRigid3(initial_trajectory_pose.relative_pose()),
                            common::FromUniversal(initial_trajectory_pose.timestamp()));
                LOG_INFO("initial_trajectory_pose: %f, %f, %f deg", initial_trajectory_pose.relative_pose().translation().x(),
                            initial_trajectory_pose.relative_pose().translation().y(),
                            SlamCommon::RadToDeg(cartographer::transform::GetYaw(transform::ToRigid3(initial_trajectory_pose.relative_pose()))));
            }
        }
        DCHECK(dynamic_cast<PoseGraph2D*>(pose_graph_.get()));
        //构造2D与3D数据整合器，用于将sensor::CollatorInterface整理的传感器数据，
        //传递给2D和3D通用的接口mapping::TrajectoryBuilderInterface
        trajectory_builders_.push_back(absl::make_unique<CollatedTrajectoryBuilder>(
                                           trajectory_options, sensor_collator_.get(), trajectory_id,
                                           expected_sensor_ids,
                                           CreateGlobalTrajectoryBuilder2D(
                                               ...
                                               local_slam_result_callback)));
        //penghu 24/4/12
        local_trajectory_ = std::move(local_trajectory_builder);
    }
...
std::map<int, int> MapBuilder::LoadState(
    ...
    while (deserializer.ReadNextSerializedData(&proto)) {
        ...
        case SerializedData::kSubmap: {
            ...
            const auto& submap2d_proto = proto.submap().submap_2d();
            if (!submap2d_proto.finished())
            {
                LOG_INFO("unfinished submap");
                continue; // skip the unfinished submaps.
            }
            const int trajectory_id = proto.submap().submap_id().trajectory_id();
            const int submap_index = proto.submap().submap_id().submap_index();
    
            const auto global_submap_pose2d =
                    cartographer::transform::Project2D(cartographer::transform::ToRigid3(
                                                            pose_graph_proto.trajectory(trajectory_id).submap(submap_index).pose()));
            const auto local_submap_pose2d =
                    cartographer::transform::Project2D(
                        cartographer::transform::ToRigid3(submap2d_proto.local_pose()));
            // LOG_INFO("submap_index: %d", submap_index);
            // LOG_INFO("global_submap_pose2d: %f, %f, %f", global_submap_pose2d.translation().x(),
            //                                              global_submap_pose2d.translation().y(),
            //                                              global_submap_pose2d.rotation().angle());
            // LOG_INFO("local_submap_pose2d: %f, %f, %f", local_submap_pose2d.translation().x(),
            //                                              local_submap_pose2d.translation().y(),
            //                                              local_submap_pose2d.rotation().angle());
                
            submap2d_protos_.push_back(submap2d_proto);
            const auto local_to_global = global_submap_pose2d * local_submap_pose2d.inverse();
            local_to_global_vec_.emplace_back(local_to_global);
            // LOG_INFO("csm local_to_global - %d: %f, %f, %f", int(local_to_global_vec_.size())-1, local_to_global.translation().x(),
            //                                              local_to_global.translation().y(),
            //                                              local_to_global.rotation().angle());
    ...
    m_file_interface->SetLocalToGlobalVec(local_to_global_vec_);
    m_file_interface->SetSubmap2dProtos(submap2d_protos_);

    double cost_time = SlamCommon::ToSeconds(SlamCommon::TimeNow() - start_time);
    LOG_INFO("LoadSubmaps: load all %d submaps cost %f s", submap2d_protos_.size(), cost_time);
